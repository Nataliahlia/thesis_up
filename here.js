    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        button.addEventListener('click', function(e) {
            ...
            // Step 2 -> AJAX list of thesis topics
            // Send a GET request to the server to fetch the thesis topics, asychronously and returns a promise
            fetch('/mythesis-details')
                ...
                .then(mythesis => {
                    ...
                    const thesis = mythesis[0]; // Array -> needs to access the elements
                    ...
                    theThesis.style.display = 'block'; // Make sure the div is visible
            })
            .catch(error => {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Step 1 -> Form, no data are send here
        ...
        // Fetch to get the data for the form
        fetch('/myprofile-edit')
            ...
            .then(studentInfo => {
                // Step 2 -> Populate the form with the fetched data
        ...
        editForm.addEventListener('submit', function(e) {
            ...
            .catch(error => {
                // Show custom alert
                showCustomAlert('Error updating profile. Please try again.');
            });
        });
    });
        ...
        button.addEventListener('click', function(e) {
            ...
            fetch('/mythesis-details')
                ...
                .then(mythesis => {
                    ...
                    const thesis = mythesis[0]; // Array -> needs to access the elements
                    window.currentThesis = thesis; // Store the thesis in a global variable for later use
                    ...
                    if (thesis.state === 'Υπό Εξέταση') {
                        ...
                        timeForm.addEventListener('submit', function (e) {   // When submitting the form do the following
                            e.preventDefault();     // Prevent the default form submission
                        ...
                        fetch(`/get-examination-info/${thesis.thesis_id}`)
                        .then(res => {
                        ...
                        .catch(err => {
                            console.error('Σφάλμα κατά το fetch των στοιχείων εξέτασης:', err);
                        });

// Εως εδω τελος

                        fetch(`/get-grades/${thesis.thesis_id}`)
                        .then(res => {
                            if (!res.ok) throw new Error('Network response was not ok');
                            return res.json();
                        })
                        .then(gradesData => {
                            ...
                            const existingNimertis = window.currentThesis?.nimertis_link || '';
                            ...
                            if (Array.isArray(grades) && grades.length > 0) {
                                ...
                                gradesInfo.innerHTML = `
                                    ...
                                    <ul>
                                    ${grades.map(grade => {
                                        const isInstructor = window.currentThesis && grade.professor_full_name === window.currentThesis.full_instructor_name;
                                    ...
                                    <div class="d-flex align-items-center mb-2" style="max-width: 750px;">
                                        ...
                                        <button class="btn btn-outline-bordeaux w-100 ms-2" onclick="downloadPDF()">
                                            Κατέβασε το Πρακτικό σε PDF
                                        </button>
                                    </div>

                                    <div class="d-flex align-items-center mb-2" style="max-width: 650px;">
                                        <div style="min-width: 420px;">
                                        ...
                                        <button id="saveNimertisBtn" class="btn btn-outline-bordeaux" ${hideSaveBtn}>
                                            Καταχώρηση
                                        </button>
                                        <button id="editNimertisBtn" class="btn btn-outline-bordeaux ms-2" ${existingNimertis ? '' : 'style="display: none;"'}>
                                            Αλλαγή
                                        </button>
                                    </div>
                                `;

                                // Add the event listener for the download button, set a timeout to ensure the button is available
                                // Because it is created using innerHTML
                                setTimeout(() => {
                                    const downloadBtn = document.getElementById('downloadProtocolBtn');
                                    if (downloadBtn) {
                                        downloadBtn.addEventListener('click', () => {
                                            const thesisId = window.currentThesis?.thesis_id;
                                            if (thesisId) {
                                ...
                                setTimeout(() => {
                                    ...
                                    if (saveBtn && linkInput) {
                                        ...
                                        saveBtn.addEventListener('click', () => {
                                            ...
                                            const thesisId = window.currentThesis?.thesis_id;   // Get the thesis id
                        ...
                        // Grades info as the last element
                        const gradesInfo = document.getElementById('gradesInfo');
                        const chronologicalInfo = document.getElementById('chronologicalInfo');

                        fetch(`/get-thesis-events/${thesis.thesis_id}`)
                        .then(res => res.json())
                        .then(data => {
                        ...
                        .catch(() => {
                            timelineList.innerHTML = '<p>Σφάλμα κατά τη φόρτωση των γεγονότων</p>';
                        });
                        ...
                        .then(professors => {
                            ...
                            // Get the existing committee invitations, to display them 
                            fetch(`/get-committee-invitations/${thesis.thesis_id}`)
                            .then(res => res.json())
                            .then(data => {
                            ...
                            addProfessorBtn.addEventListener('click', () => {
                                ...
                                fetch('/send-committee-invitation', {
                                    method: 'POST',
        ...
        form.addEventListener('submit', function (e) {
            ...
            const filePreviouslyUploaded = !!(window.currentThesis && window.currentThesis.draft_file);
                ...
                if (result.success) {
                    ...
                    fetch('/mythesis-details')
                        ...
                        .then(data => {
                            ...
                            window.currentThesis = updatedThesis;
    ...
    // Used to handle the button that enables the editing of the examination info   
...
// Κατω απο εδω τελος 
// <!-- This is first created for the editing form, could/will be used for other cases as well -->
    ...
    // This is the function for the file upload
    function renderUploadedFile(filename) {
        const fileInput = document.getElementById('progressFile');  // Find the file element in the form in order to handle it handle it
    ...
    function downloadPDF() {
        const thesisId = window.currentThesis?.thesis_id;   // Get the thesis ID from the global variable
        // Validate the thesis ID
        if (!thesisId) {
    ...
    // Handle the display and the hiding of the back button
    document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('DOMContentLoaded', initializeLogoutCustom);

//<!-- This handles the starting page and the page reloads -->
